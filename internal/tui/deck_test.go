...
func TestDecksList(t *testing.T) {
	...
	t.Run(
		"shows homepage with many decks", func(t *testing.T) {
			m := newTestModel(t, manyDecks, tui.WithClock(clock.New(oldestCard.LastReview.Add(24*time.Hour*4)))).
				Init().
				Get()
	...
	t.Run(
		"does not start review when it does not have due cards", func(t *testing.T) {
			view := newTestModel(t, manyDecks, tui.WithClock(clock.Clock{Time: oldestCard.LastReview})).
				Init().

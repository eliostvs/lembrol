...
import (
	...
	"github.com/eliostvs/lembrol/internal/clock/test"
...
func TestNewReview(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			clock: test.New(beforeOldestCard),
			deck:  largeDeck,
			want:  0,
		},
		{
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				review := flashcard.NewReview(
					newDeck(
						t, tt.deck, func(o *option) {
							o.clock = tt.clock
						},
					), tt.clock,
				)

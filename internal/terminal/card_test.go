...
func TestCardsList(t *testing.T) {
	...
	t.Run(
		"does not start review when it does not have due cards", func(t *testing.T) {
			m, _ := newTestModel(singleCardDeck, terminal.WithClock(clock.Clock{Time: latestCard.ReviewedAt})).
				Init().
				SendKeyType(tea.KeyEnter).
...
func TestCardCreate(t *testing.T) {
	...
	t.Run(
		"shows deck when the creation is canceled", func(t *testing.T) {
			m, _ := newTestModel(manyDecks).
				Init().
				SendKeyType(tea.KeyEnter).
				SendKeyRune(createKey).
	...
	t.Run(
		"validates card creation", func(t *testing.T) {
			t.Run(
				"missing question", func(t *testing.T) {
					m, _ := newTestModel(test.TempDirCopy(t, emptyDeck)).
						Init().
						SendKeyType(tea.KeyEnter).
						SendKeyRune(createKey).
			...
			t.Run(
				"missing answer", func(t *testing.T) {
					m, _ := newTestModel(test.TempDirCopy(t, emptyDeck)).
						Init().
						SendKeyType(tea.KeyEnter).
						SendKeyRune(createKey).
	...
	t.Run(
		"write multiline text", func(t *testing.T) {
			m, _ := newTestModel(test.TempDirCopy(t, emptyDeck)).
				Init().
				SendKeyType(tea.KeyEnter).
				SendKeyRune(createKey).
